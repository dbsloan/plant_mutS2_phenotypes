...
mutS2_data <- read.csv("discord.summary.mod.csv")
...
ggplot(data=mutS2_data, aes(y=DiscordantRatePerMM, x=Genotype)) +
  ...
  ylim(0,1150) +
  ylab ("Plastome Discordant Read Pairs") +
...
ggsave ("Discord_plot.pdf", width=3.25, height=2.5)
...
anova_model <- lm(log10(DiscordantRatePerMM) ~ Treatment + Genotype + Treatment:Genotype, data = mutS2_data)
