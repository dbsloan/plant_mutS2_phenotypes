...
pos_data_mutS2 = read.table ("hotspots.mutS2.mito.txt", header=TRUE)
pos_data_WT = read.table ("hotspots.WT.mito.txt", header=TRUE)
...
ggplot(data=pos_data, aes(x=Position, y=Read_Count)) +
  ...
  theme_bw() +
  xlab ("Position in Mitogenome Genome (bp)") +
...
ggsave ("Hotspot_plot.mito.pdf", width=3.25, height=3.25)